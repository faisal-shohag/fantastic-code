...
const getSubmissions = async (userId, problemId) => {
  try {
    const response = await fetch(`/api/submission/${userId}/${problemId}`, {cache: 'no-store'});
  ...
  return (
    <div >
      {data?.map((submission) => (
       ...
       <Link  href={`submissions/${submission.id}`}> <div
          
          className="p-3 mt-1 border grid grid-cols-4 items-center"
        >
          <div>
            <h1
          ...
          <div className="text-xs">
            <Badge className="dark:bg-zinc-700 text-muted-foreground">
              {" "}
              {submission.language}
            </Badge>
          </div>
